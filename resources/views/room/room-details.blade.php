    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="w-full lg:w-1/2">
            ...
            <div class="rounded-lg overflow-hidden shadow-lg">
                <img src="{{ $imageUrls[0] ?? asset('images/no-image.png') }}" class="w-full h-auto object-cover"
                    alt="{{ $room[0]['name'] }}" />
            </div>
            @endif
            @if (isset($room[1]['image']))
            <!-- Small images beneath the main image -->
            <div class="grid grid-cols-4 gap-2 mt-4">
                @for ($i = 1; $i <= 4; $i++)
                    @if(isset($room[$i]['image']))
                    <div class="rounded-lg overflow-hidden shadow-lg h-32 w-32">
                    <img src="{{ $imageUrls[$i] ?? asset('images/no-image.png') }}" class="object-cover p-auto w-full h-full"
                        alt="Image {{ $i }}" />
            </div>
            @else
            @php break; @endphp
